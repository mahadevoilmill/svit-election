...
app.post('/login', async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, role: data.role || 'user' });
  } catch (error) {
...
app.post('/votes/by-sr/vote', async (req, res) => {
  ...
  try {
    ...
    // Record individual ballot
    const { error: ballotError } = await supabase.from('ballots').insert({ sr_numbers: [String(sr_number)] });
    if (ballotError) console.error('❌ Failed to record ballot:', ballotError.message);
...
app.post('/votes/by-sr/votes', async (req, res) => {
  ...
  try {
    ...
    // Record individual ballot for multiple SR numbers
    if (processedSrNumbers.length > 0) {
      const { error: ballotError } = await supabase.from('ballots').insert({ sr_numbers: processedSrNumbers });
      if (ballotError) console.error('❌ Failed to record multiple ballot:', ballotError.message);
    }
